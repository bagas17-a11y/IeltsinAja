...
export default function WritingModule() {
  ...
  const downloadWritingPDF = (feedbackData: any, essayText: string, taskType: string) => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Writing Feedback — Mumpune</title>
...
<body><h1>IELTS Writing Feedback</h1><p>Mumpune &mdash; ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
...
${modelHtml}</body></html>`;
  ...
  const FeedbackDisplay = ({ feedbackData, isRevisionFeedback = false }: { feedbackData: any; isRevisionFeedback?: boolean }) => (
    <div className="space-y-5">
      ...
      <button
        onClick={() => {
          const text = `I just scored Band ${feedbackData.overallBand} on IELTS Writing on Mumpune! 🎯`;
