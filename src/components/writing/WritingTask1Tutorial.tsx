  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <h2 className="text-xl font-bold mt-2 text-gray-900">Read the Chart First</h2>
        <p className="text-sm text-gray-500 mt-1">Before writing a word, spend 2 minutes reading the data. Click each observation below.</p>
      </motion.div>
        ...
        {observations.map((obs, i) => (
          <motion.button key={i} variants={popIn(0.2 + i * 0.08)} initial="hidden" animate="visible"
            ...
            className={`w-full text-left p-4 rounded-xl border transition-all duration-300 ${
              revealed.includes(i) ? "bg-accent/10 border-accent/40" : "bg-gray-50 border-gray-200 hover:border-gray-300"
            }`}>
            <div className="flex items-start gap-3">
              <div className={`w-5 h-5 rounded-full shrink-0 mt-0.5 flex items-center justify-center transition-colors ${
                revealed.includes(i) ? "bg-accent" : "bg-gray-200"
              }`}>
                {revealed.includes(i) ? <Check className="w-3 h-3 text-white" /> : <span className="text-[10px] text-gray-500">{i + 1}</span>}
              </div>
              <div>
                <p className="text-sm font-medium text-gray-900">{obs.q}</p>
                <AnimatePresence>
                  {revealed.includes(i) && (
                    <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                      exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }}
                      className="text-xs text-accent mt-1.5 overflow-hidden">
                      {obs.a}
                    </motion.p>
                  )}
                </AnimatePresence>
      ...
      {revealed.length < 3 && (
        <p className="text-xs text-center text-gray-400">Click all 3 observations to continue ({revealed.length}/3)</p>
      )}
    </div>
  );
}
...
function Slide3Blueprint({ onComplete }: { onComplete: () => void }) {
  const parts = [
    { title: "Introduction", badge: "1 sentence", bg: "bg-blue-50 border-blue-200 hover:bg-blue-100", numBg: "bg-blue-200", numText: "text-blue-800", titleText: "text-blue-900", descText: "text-blue-800", badgeText: "text-blue-500 border-blue-200", text: "Paraphrase the question. Mention: visual type, what it shows, time period, units. Never copy the question word-for-word." },
    { title: "Overview", badge: "1–2 sentences", bg: "bg-purple-50 border-purple-200 hover:bg-purple-100", numBg: "bg-purple-200", numText: "text-purple-800", titleText: "text-purple-900", descText: "text-purple-800", badgeText: "text-purple-500 border-purple-200", text: "The big picture — main trends only, no figures. Who wins overall? What is highest/lowest? What is the biggest difference?" },
    { title: "Body Paragraph 1", badge: "Key group", bg: "bg-green-50 border-green-200 hover:bg-green-100", numBg: "bg-green-200", numText: "text-green-800", titleText: "text-green-900", descText: "text-green-800", badgeText: "text-green-500 border-green-200", text: "Pick a pattern (e.g. categories where UK > France). Use 2–3 specific figures with comparison language." },
    { title: "Body Paragraph 2", badge: "Other group", bg: "bg-amber-50 border-amber-200 hover:bg-amber-100", numBg: "bg-amber-200", numText: "text-amber-800", titleText: "text-amber-900", descText: "text-amber-800", badgeText: "text-amber-500 border-amber-200", text: "The contrasting pattern (e.g. France similar/higher). Link from BP1 with 'On the other hand' or similar." },
  ];
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <h2 className="text-xl font-bold mt-2 text-gray-900">The 4-Part Structure</h2>
        <p className="text-sm text-gray-500 mt-1">Every Band 7+ Task 1 uses this structure. Click each part to understand its role.</p>
      </motion.div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full ${p.numBg} flex items-center justify-center shrink-0`}>
                    <span className={`text-xs font-bold ${p.numText}`}>{i + 1}</span>
                  </div>
...
function Slide4Introduction({ onComplete }: { onComplete: () => void }) {
  ...
  const annotations = [
    { phrase: "illustrates", note: "paraphrase of 'shows'", color: "text-blue-700 bg-blue-100 border-blue-300" },
    { phrase: "amount of money spent on", note: "paraphrase of 'expenditure'", color: "text-green-700 bg-green-100 border-green-300" },
    { phrase: "France and the UK", note: "names both countries explicitly", color: "text-purple-700 bg-purple-100 border-purple-300" },
    { phrase: "Units are measured in pounds sterling.", note: "adds the unit — examiners expect this!", color: "text-amber-700 bg-amber-100 border-amber-300" },
  ];
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <p className="text-sm text-gray-500 mt-1">Paraphrase the question in one sentence. Never copy it word-for-word.</p>
      </motion.div>

      <motion.div variants={fadeUp(0.1)} initial="hidden" animate="visible"
        className="p-4 rounded-xl bg-gray-50 border border-gray-200">
        <p className="text-[11px] uppercase tracking-wide text-gray-400 mb-2">Original question</p>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-4">
            <div className="p-4 rounded-xl bg-accent/10 border border-accent/30">
              ...
              <p className="text-sm text-gray-800 leading-relaxed">
                "The chart{" "}
                <span className="font-semibold text-blue-700 bg-blue-100 px-1 rounded">illustrates</span>{" "}
                the{" "}
                <span className="font-semibold text-green-700 bg-green-100 px-1 rounded">amount of money spent on</span>{" "}
                five consumer goods (cars, computers, books, perfume and cameras) in{" "}
                <span className="font-semibold text-purple-700 bg-purple-100 px-1 rounded">France and the UK</span>{" "}
                in 2010.{" "}
                <span className="font-semibold text-amber-700 bg-amber-100 px-1 rounded">Units are measured in pounds sterling.</span>"
              </p>
            </div>
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <h2 className="text-xl font-bold mt-2 text-gray-900">Write the Overview</h2>
        <p className="text-sm text-gray-500 mt-1">The make-or-break paragraph for Band 7+. Big picture only — no specific numbers. Tick each element to build it.</p>
      </motion.div>
      ...
      <div className="space-y-2.5">
        {items.map((item, i) => (
          <motion.div key={i} variants={fadeUp(0.1 + i * 0.08)} initial="hidden" animate="visible">
            <button onClick={() => toggle(i)}
              className={`w-full text-left p-4 rounded-xl border transition-all duration-300 ${
                checked.includes(i) ? "bg-green-50 border-green-300" : "bg-gray-50 border-gray-200 hover:border-green-300"
              }`}>
              <div className="flex items-center gap-3">
                <div className={`w-5 h-5 rounded border-2 shrink-0 flex items-center justify-center transition-all ${
                  checked.includes(i) ? "bg-green-500 border-green-500" : "border-gray-400"
                }`}>
                  {checked.includes(i) && <Check className="w-3 h-3 text-white" />}
                </div>
                <span className={`text-sm font-medium ${checked.includes(i) ? "text-green-700" : "text-gray-900"}`}>{item.check}</span>
              </div>
              <AnimatePresence>
                {checked.includes(i) && (
                  <motion.p initial={{ opacity: 0, height: 0, marginTop: 0 }}
                    animate={{ opacity: 1, height: "auto", marginTop: 8 }}
                    transition={{ duration: 0.25 }}
                    className="text-xs text-gray-600 italic pl-8 overflow-hidden leading-relaxed">
                    "{item.sentence}"
                  </motion.p>
                )}
              </AnimatePresence>
      ...
      {checked.length < 3 && (
        <p className="text-xs text-center text-gray-400">Tick all 3 elements to reveal the full overview ({checked.length}/3)</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <h2 className="text-xl font-bold mt-2 text-gray-900">Group Your Data</h2>
        <p className="text-sm text-gray-500 mt-1">Never describe every bar in order. Group by pattern. Click each category to drop it into the correct group.</p>
      </motion.div>

      <ChartRef />

      <motion.div variants={fadeUp(0.1)} initial="hidden" animate="visible"
        className="flex flex-wrap gap-2 p-4 rounded-xl bg-gray-50 border border-gray-200 min-h-[56px]">
        <p className="text-xs text-gray-400 w-full mb-1">Click to assign:</p>
        <AnimatePresence>
          {CATEGORIES.filter(c => assigned[c] === undefined).map(cat => (
            <motion.button key={cat} layout exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }}
              onClick={() => assign(cat)}
              className="px-3 py-1.5 rounded-lg bg-white border border-gray-300 text-sm font-medium text-gray-700 hover:border-accent/50 hover:bg-accent/5 transition-all">
              {cat}
            </motion.button>
          ))}
        </AnimatePresence>
        {Object.keys(assigned).length === 5 && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs text-green-600 flex items-center gap-1 w-full">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: "UK spent more", desc: "→ Body P1", bg: "bg-blue-50 border-blue-200", labelText: "text-blue-900", descText: "text-blue-400", chipBg: "bg-blue-100 text-blue-800 border-blue-300" },
          { label: "France similar / more", desc: "→ Body P2", bg: "bg-purple-50 border-purple-200", labelText: "text-purple-900", descText: "text-purple-400", chipBg: "bg-purple-100 text-purple-800 border-purple-300" },
        ].map((g, gi) => (
          <motion.div key={gi} variants={fadeUp(0.2 + gi * 0.05)} initial="hidden" animate="visible"
            ...
            <p className={`text-xs font-semibold mb-0.5 ${g.labelText}`}>{g.label}</p>
            <p className={`text-[10px] mb-2.5 ${g.descText}`}>{g.desc}</p>
            <div className="flex flex-wrap gap-1.5">
              {CATEGORIES.filter(c => assigned[c] === gi).map(cat => (
                <motion.span key={cat} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                  className={`px-2 py-0.5 rounded-md text-xs font-medium border ${g.chipBg}`}>
                  {cat}
                </motion.span>
              ))}
            </div>
...
const BODY1_SENTENCES: SentenceData[] = [
  {
    ...
    highlights: [
      { phrase: "In terms of", tag: "TOPIC OPENER", color: "text-blue-700 bg-blue-100" },
      { phrase: "as opposed to", tag: "COMPARISON", color: "text-green-700 bg-green-100" },
    ],
  },
  {
    text: "Similarly, the British expenditure was higher on books than the French (around £400,000 and £300,000 respectively).",
    highlights: [
      { phrase: "Similarly,", tag: "LINKER", color: "text-purple-700 bg-purple-100" },
      { phrase: "respectively", tag: "DATA TECHNIQUE", color: "text-amber-700 bg-amber-100" },
    ],
  },
  {
    text: "In the UK, expenditure on cameras (just over £350,000) was over double that of France, which was only £150,000.",
    highlights: [
      { phrase: "over double that of", tag: "STRONG COMPARISON", color: "text-red-700 bg-red-100" },
      { phrase: "just over", tag: "APPROXIMATION", color: "text-cyan-700 bg-cyan-100" },
    ],
  },
];
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <h2 className="text-xl font-bold mt-2 text-gray-900">Build Body Paragraph 1</h2>
        <p className="text-sm text-gray-500 mt-1">Group: UK spent more (Cars, Books, Cameras). Click "Add sentence" — watch each phrase build the paragraph.</p>
      </motion.div>

      <ChartRef />

      <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 space-y-3 min-h-[100px]">
      ...
      {revealed < BODY1_SENTENCES.length ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5 justify-center">
            {[
              { tag: "TOPIC OPENER", c: "text-blue-700 border-blue-300" },
              { tag: "LINKER", c: "text-purple-700 border-purple-300" },
              { tag: "COMPARISON", c: "text-green-700 border-green-300" },
              { tag: "APPROXIMATION", c: "text-cyan-700 border-cyan-300" },
              { tag: "STRONG COMPARISON", c: "text-red-700 border-red-300" },
              { tag: "DATA TECHNIQUE", c: "text-amber-700 border-amber-300" },
            ].map(b => (
...
const BODY2_SENTENCES: SentenceData[] = [
  {
    ...
    highlights: [
      { phrase: "On the other hand,", tag: "CONTRAST LINKER", color: "text-purple-700 bg-purple-100" },
      { phrase: "the remaining goods", tag: "GROUPING REFERENCE", color: "text-blue-700 bg-blue-100" },
    ],
  },
  {
    ...
    highlights: [{ phrase: "slightly more than", tag: "COMPARISON", color: "text-green-700 bg-green-100" }],
  },
  {
    ...
    highlights: [
      { phrase: "Neither of the countries", tag: "INCLUSIVE COMPARISON", color: "text-amber-700 bg-amber-100" },
      { phrase: "accounted for", tag: "FORMAL VERB", color: "text-cyan-700 bg-cyan-100" },
    ],
  },
];
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <h2 className="text-xl font-bold mt-2 text-gray-900">Build Body Paragraph 2</h2>
        <p className="text-sm text-gray-500 mt-1">Group: France similar or more (Computers, Perfume). Start with the contrast linker to signal the shift.</p>
      </motion.div>
...
const MODEL_PARAGRAPHS = [
  { label: "Introduction", color: "border-blue-300 bg-blue-50", labelColor: "text-blue-700", text: "The chart illustrates the amount of money spent on five consumer goods (cars, computers, books, perfume and cameras) in France and the UK in 2010. Units are measured in pounds sterling." },
  { label: "Overview", color: "border-purple-300 bg-purple-50", labelColor: "text-purple-700", text: "Overall, the UK spent more money on consumer goods than France in the period given. Both the British and the French spent most of their money on cars whereas the least amount of money was spent on perfume in the UK compared to cameras in France. Furthermore, the most significant difference in expenditure between the two countries was on cameras." },
  { label: "Body Paragraph 1", color: "border-green-300 bg-green-50", labelColor: "text-green-700", text: "In terms of cars, people in the UK spent about £450,000 on this as opposed to the French at £400,000. Similarly, the British expenditure was higher on books than the French (around £400,000 and £300,000 respectively). In the UK, expenditure on cameras (just over £350,000) was over double that of France, which was only £150,000." },
  { label: "Body Paragraph 2", color: "border-amber-300 bg-amber-50", labelColor: "text-amber-700", text: "On the other hand, the amount of money paid out on the remaining goods was higher in France. Above £350,000 was spent by the French on computers which was slightly more than the British who spent exactly £350,000. Neither of the countries spent much on perfume which accounted for £200,000 of expenditure in France but under £150,000 in the UK." },
];
  ...
  return (
    <div className="space-y-5">
      <motion.div variants={fadeUp(0)} initial="hidden" animate="visible">
        ...
        <p className="text-sm text-gray-500 mt-1">Click "Reveal" to see how all four paragraphs come together.</p>
      </motion.div>
        ...
        <AnimatePresence>
          {MODEL_PARAGRAPHS.slice(0, revealed).map((p, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38 }}
              ...
              <p className="text-sm text-gray-700 leading-relaxed italic">"{p.text}"</p>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          ...
          <div className="flex gap-2 justify-center flex-wrap">
            <a href="/assets/Task-1-Word-List.pdf" target="_blank" rel="noopener noreferrer">
              <Button variant="outline" size="sm" className="gap-1.5 text-xs border-gray-300 text-gray-700 hover:bg-gray-100">
